...
Only enable =xclip-mode= for TTY frames. The =emacs-pgtk= GUI handles the
Wayland clipboard natively, and routing it through =xclip= (which uses
XWayland) breaks =p= / =P= because XWayland's clipboard is separate from
=wl-clipboard=. With =services.emacs.enable= we run a daemon, so use a frame
hook instead of a one-shot init check.

For TTY frames, point =xclip= at =wl-copy= so terminal Emacs shares the same
clipboard as native Wayland apps.

#+begin_src emacs-lisp
(defun jag/maybe-enable-xclip (&optional frame)
  "Enable `xclip-mode' only in TTY frames; pgtk handles GUI clipboard natively."
