...
Configure ACP-backed Claude and Codex agents. Pi is temporarily disabled because streaming through `agent-shell` can freeze Emacs.
...
(defvar-local jag/agent-shell-viewport-queue-mode nil
  "When non-nil, viewport `compose-send' enqueues instead of submitting.
A value of t means \"queue a new request\".  A list of the form
(:original ORIG) means \"edit the queued entry whose text is ORIG\".")
(put 'jag/agent-shell-viewport-queue-mode 'permanent-local t)

(defun jag/agent-shell--clear-queue-flag (&rest _)
  "Reset the queue-compose flag in the current viewport buffer."
  (when (or (derived-mode-p 'agent-shell-viewport-edit-mode)
            (derived-mode-p 'agent-shell-viewport-view-mode))
    (setq jag/agent-shell-viewport-queue-mode nil)))

(defun jag/agent-shell--update-pending-at (shell-buffer original prompt)
  "In SHELL-BUFFER, replace the pending entry equal to ORIGINAL with PROMPT.
Return the position replaced, or nil if ORIGINAL is no longer pending."
  (with-current-buffer shell-buffer
    (let* ((pending (map-elt agent-shell--state :pending-requests))
           (pos (seq-position pending original #'equal)))
      (when pos
        (let ((updated (append (seq-take pending pos)
                               (list prompt)
                               (seq-drop pending (1+ pos)))))
          (map-put! agent-shell--state :pending-requests updated))
        (agent-shell--display-pending-requests))
      pos)))

(defun jag/agent-shell-queue-request-compose ()
  "Compose a request via the viewport buffer; submit enqueues if shell is busy.
Routes through `agent-shell-queue-request' so busy/idle behavior is preserved."
  (interactive)
  ...
  (let* ((shell-buffer (current-buffer))
    ...
    (when-let ((viewport (agent-shell-viewport--buffer
      ...
      (with-current-buffer viewport
        (setq jag/agent-shell-viewport-queue-mode t)))))

(defun jag/agent-shell-edit-queued-request ()
  "Pick a pending queued request and edit it in the compose viewport.
On submit, replaces the original at its position, or re-enqueues if the
original entry has already been resumed."
  (interactive)
  (unless (derived-mode-p 'agent-shell-mode)
    (user-error "Not in an agent-shell buffer"))
  (let ((pending (map-elt agent-shell--state :pending-requests)))
    ...
    (let* ((shell-buffer (current-buffer))
           (choices (seq-map-indexed
                     (lambda (req idx)
                       (cons (format "%d: %s"
                                     (1+ idx)
                                     (truncate-string-to-width req 80 nil nil "..."))
                             req))
                     pending))
           (selection (completing-read "Edit queued request: "
                                       (mapcar #'car choices) nil t))
           (original (cdr (assoc selection choices)))
           (viewport (agent-shell-viewport--buffer :shell-buffer shell-buffer))
           (display-action
            '((display-buffer-reuse-window display-buffer-in-side-window)
              (side . bottom)
              (slot . 0)
      ...
      (with-current-buffer viewport
        (when (and (> (buffer-size) 0)
                   ...
                   (not (y-or-n-p "Compose buffer has content.  Replace? ")))
          (user-error "Aborted"))
        (agent-shell-viewport-edit-mode)
        (agent-shell-viewport--initialize)
        (insert original)
        (setq jag/agent-shell-viewport-queue-mode (list :original original)))
      (display-buffer viewport display-action)
      (when-let ((win (get-buffer-window viewport)))
        (select-window win)))))

(defun jag/agent-shell-viewport-compose-send-around (orig &rest args)
  "Route `agent-shell-viewport-compose-send' to the queue when flagged."
  (if (and (derived-mode-p 'agent-shell-viewport-edit-mode)
           jag/agent-shell-viewport-queue-mode)
      (let* ((mode jag/agent-shell-viewport-queue-mode)
        ...
        (cond
         ((and (consp mode) (eq (car mode) :original))
          (let ((pos (jag/agent-shell--update-pending-at
                      shell-buffer (cadr mode) prompt)))
            (if pos
                (message "Pending request %d updated" (1+ pos))
              (with-current-buffer shell-buffer
...
(advice-add 'agent-shell-viewport-compose-send :around
            #'jag/agent-shell-viewport-compose-send-around)
(advice-add 'agent-shell-viewport-compose-cancel :before
            #'jag/agent-shell--clear-queue-flag)
...
(map! :leader
      ...
      :desc "Agent shell Pi disabled" "j a p" #'jag/agent-shell-pi-disabled
      :desc "Agent shell queue request" "j a q" #'jag/agent-shell-queue-request-compose
      :desc "Agent shell edit queued" "j a Q" #'jag/agent-shell-edit-queued-request
      :desc "Agent shell switch session" "j a s" #'jag/agent-shell-switch-session
      :desc "Agent shell switch project session" "j a S" #'jag/agent-shell-switch-project-session
      :desc "Agent shell switch workspace session" "j a w" #'jag/agent-shell-switch-workspace-session
