export const meta = {
  name: 'deep-research-staged',
  description: 'Deep research harness with staged verification waves (rate-limit safe) — fan-out web searches, fetch sources, adversarially verify claims in sequential batches, synthesize a cited report.',
...
// Question is passed via Workflow({name: 'deep-research-staged', args: '<question>'}).
...
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({name: 'deep-research-staged', args: '<question>'})." }
}
...
phase("Verify")
// Staged waves: verify a few claims at a time instead of all 25 panels at once,
// so the request rate stays under the API's server-side throttle.
const VERIFY_BATCH = 4 // claims per wave (× VOTES_PER_CLAIM votes in flight)
const voted = []
for (let batchStart = 0; batchStart < rankedClaims.length; batchStart += VERIFY_BATCH) {
  const wave = rankedClaims.slice(batchStart, batchStart + VERIFY_BATCH)
  log("verify wave " + (Math.floor(batchStart / VERIFY_BATCH) + 1) + "/" + Math.ceil(rankedClaims.length / VERIFY_BATCH) + " (" + wave.length + " claims)")
