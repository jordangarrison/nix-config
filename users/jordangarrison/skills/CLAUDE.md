...
- Upstream skills are declared in `programs.agent-skills.external` in `users/jordangarrison/home.nix`. For a skill that documents a CLI, take it from that CLI's package so the versions match. For any other skill, add a `flake = false` input. Never install skills with `npx skills`, `pup skills install`, `readwise skills install`, `flo skills add`, or by copying files by hand: Home Manager refuses to overwrite such directories, so the next switch fails.
