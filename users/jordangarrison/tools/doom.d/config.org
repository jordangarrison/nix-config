...
- [[#editor-behavior][Editor Behavior]]
  - [[#debug-on-quit-toggle][Debug on Quit Toggle]]
...
- [[#aillm-integration][AI/LLM Integration]]
  ...
  - [[#vterm-agent-sessions][VTerm Agent Sessions]]
...
(setq doom-font (font-spec :family "Fira Code"
                           :size (if (member (system-name) '("endeavour" "opportunity")) 14 14)
                           :weight 'semibold))
#+end_src

#+RESULTS:
: #<font-spec nil nil Fira\ Code nil nil semibold nil nil 15 nil nil nil nil>
...
* Editor Behavior
** Debug on Quit Toggle
Provide a quick toggle for ~debug-on-quit~. Leaving it on globally makes every
~C-g~ / ~ESC~ in vertico (and elsewhere) drop a backtrace, which is too noisy.
Use ~M-x jag/toggle-debug-on-quit~ before reproducing a daemon hang to capture a
backtrace identifying what was running, then toggle it back off.

#+begin_src emacs-lisp
(defun jag/toggle-debug-on-quit ()
  "Toggle `debug-on-quit' and report the new state."
  (interactive)
  (setq debug-on-quit (not debug-on-quit))
  (message "debug-on-quit %s" (if debug-on-quit "ON" "off")))
#+end_src
...
Mouse support in TTY frames and system clipboard integration for both TTY
and pgtk GUI frames on Wayland.

In TTY frames =xclip-mode= covers clipboard reads/writes (its =cl-defmethod=
dispatchers are gated on =(window-system nil)=, so they fire there). In
=pgtk= GUI frames the default =gui-selection-value= runs, and on Wayland it
caches stale selections — a region yanked inside Emacs keeps coming back
as the "clipboard" content even after copying fresh text from another app.

Fix: install =wl-paste= / =wl-copy= shims as
=interprogram-{paste,cut}-function= globally on Wayland. They work for both
GUI and TTY frames; xclip-mode's dispatchers still handle the TTY-specific
=gui-backend-get-selection= path internally.

For SSH TTY use, the daemon shells out to its *own* Wayland clipboard.
Cross-machine paste relies on terminal bracketed-paste mode (built-in);
copy relies on OSC 52, enabled via Doom's =:os (tty +osc)=.

#+begin_src emacs-lisp
(defun jag/maybe-enable-xclip (&optional frame)
  "Enable mouse + xclip-mode in TTY frames; pgtk GUI uses wl-paste shims."
  (with-selected-frame (or frame (selected-frame))
    (unless (display-graphic-p)
      (unless (bound-and-true-p xterm-mouse-mode) (xterm-mouse-mode 1))
      (unless (bound-and-true-p xclip-mode) (xclip-mode 1)))))

(add-hook 'after-make-frame-functions #'jag/maybe-enable-xclip)
(jag/maybe-enable-xclip)
...
(defun jag/wl-paste ()
  "Read the system CLIPBOARD via `wl-paste'. Return nil when empty."
...
(defun jag/wl-copy (text)
  "Write TEXT to the system CLIPBOARD via `wl-copy'.
Use `call-process-region' (sync) instead of `start-process'; async
`process-send-string'+`process-send-eof' from the emacs daemon does not
reliably deliver to wl-copy and leaves hung child processes."
  (when (and text (executable-find "wl-copy"))
    (with-temp-buffer
      (insert text)
...
(when (and (eq system-type 'gnu/linux)
           (getenv "WAYLAND_DISPLAY"))
  (setq interprogram-paste-function #'jag/wl-paste
        interprogram-cut-function   #'jag/wl-copy))

;; Doom's `:os (tty +osc)' loads clipetty and turns on `global-clipetty-mode',
;; which adds an `:around' advice on `interprogram-cut-function'. That advice
;; fires in pgtk GUI frames too and emits OSC 52 to a terminal that isn't
;; there, swallowing the kill before `jag/wl-copy' can run. Disable the global
;; mode and enable clipetty only when a TTY frame attaches (e.g. SSH client),
;; which is the case OSC 52 is intended for.
(after! clipetty
...
#+begin_src emacs-lisp
(after! lsp-mode
  ;; Avoid nil recursively evaluating every flake input for completion. Large
  ;; inputs can exhaust Nix's evaluator memory and produce multi-gigabyte cores.
...
(defun jag/agent-vterm--nearest-project-marker-root (&optional directory)
  "Return nearest root at or above DIRECTORY marked by `.projectile' or `.git'.

This keeps `.projectile' useful as an override while still allowing closer Git
worktrees, whose `.git' marker is usually a file, to win over parent roots."
  (let ((dir (jag/agent-vterm--normalize-dir (or directory default-directory)))
        found)
    (while (and dir (not found))
      (if (or (file-exists-p (expand-file-name ".projectile" dir))
...
(defun jag/agent-vterm--current-project-root ()
  ...
  (jag/agent-vterm--normalize-dir
   (or (jag/agent-vterm--nearest-project-marker-root)
...
* IRC
Doom's =:app irc= module uses [[https://github.com/emacs-circe/circe][circe]] (not ERC). Connects to libera.chat
over TLS and autojoins =#emacsatx=. Open with ~SPC o I~ (~=irc~).

NickServ password is read from =~/.authinfo= when present:

#+begin_example
machine irc.libera.chat login jordangarrison password YOUR_NICKSERV_PASS
#+end_example

If no entry exists, circe still connects unauthenticated — fine for an
unregistered nick. Register later with =/msg NickServ REGISTER <pw> <email>=.
...
(after! circe
  (set-irc-server! "Libera.Chat"
    `(:host "irc.libera.chat"
      ...
      :nick "jordangarrison"
      :user "jordangarrison"
      :realname "Jordan Garrison"
      :sasl-username "jordangarrison"
...
* Org Mode
** Readwise
Disabled 2026-05-05: ~org-readwise-debug-level 1~ caused the package to dump
the entire highlight payload through ~message~ on each sync, growing
~*Messages*~ unbounded and bloating the daemon (29.7G peak before a freeze).
Re-enable only after upstream stops echoing payload data, or set
~org-readwise-debug-level 0~ if reactivating.
